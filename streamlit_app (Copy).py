...
def find_projects(base_dir: Path) -> dict[str, Path]:
    ...
    for entry in sorted(base_dir.iterdir(), key=lambda e: (not e.name.isdigit(), e.name)):
        if not entry.is_dir():
            continue
...
# ---------------------------------------------------------------------------
# Module loader
# Loads a main.py as a module WITHOUT executing top-level code, so we can
# call main() or run() explicitly.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Core runner — calls a named function inside the module
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Graph summary extraction (unchanged logic, now calls main() instead of
# running the whole file so it doesn't break on scripts without top-level code)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Per-experiment custom input widgets
# Returns (params_ready: bool, params: dict)
# params dict is passed as **kwargs to run()
# ---------------------------------------------------------------------------
...
def render_custom_input(exp_number: str) -> tuple[bool, dict]:
    ...
    # ------------------------------------------------------------------
    # Experiment-specific overrides — slot in special widgets here
    # ------------------------------------------------------------------
    if exp_number == "1":
...
def _render_visjs_canvas(canvas_key: str, weighted: bool = False) -> tuple[bool, dict]:
    ...
    result_key  = f"visjs_result_{canvas_key}"
    json_input_key = f"visjs_json_{canvas_key}"
    weighted_js = "true" if weighted else "false"
...
<div id="out-wrap" style="display:none">
  <div id="out-label">Paste this into the JSON field below ↓</div>
...
<script>
const WEIGHTED={weighted_js};
let nc=0,mode=null,src=null;
const N=new vis.DataSet(),E=new vis.DataSet();
const net=new vis.Network(document.getElementById('net'),{{nodes:N,edges:E}},{{
  physics:false,
  interaction:{{hover:true,dragNodes:true,dragView:true,multiselect:false, keyboard: false}},
  nodes:{{shape:'ellipse',size:18,
    font:{{size:13,color:'#1e40af'}},borderWidth:1.5,
    color:{{background:'#dbeafe',border:'#3b82f6',
            highlight:{{background:'#bfdbfe',border:'#2563eb'}}}}}},
  edges:{{font:{{size:11,color:'#6b7280',align:'middle'}},
    color:{{color:'#9ca3af',highlight:'#374151'}},width:1.5,smooth:false}}
}});
const pill=document.getElementById('pill');
function setMode(m){{
  mode=m;src=null;net.unselectAll();
  document.getElementById('btn-node').className=m==='node'?'active':'';
  document.getElementById('btn-edge').className=m==='edge'?'active':'';
  pill.textContent=m==='node'?'click canvas to place node':'click source node, then target';
}}
net.on('click',function(p){{
  if(mode==='node'){{
    ...
    if(p.nodes.length===1){{
      const v=p.nodes[0];
      if(src===null){{src=v;net.selectNodes([v]);pill.textContent='now click target node';}}
      else{{
        if(v!==src){{
          let lbl='';
          if(WEIGHTED){{const w=prompt('Edge weight:','1');if(w===null){{src=null;return;}}lbl=w.trim();}}
          E.add({{from:src,to:v,label:lbl}});
        }}
        src=null;net.unselectAll();pill.textContent='click source node, then target';
      }}
    }}
  }}
}});
function delSel(){{const s=net.getSelection();N.remove(s.nodes);E.remove(s.edges);}}
function clearAll(){{
  N.clear();E.clear();nc=0;src=null;mode=null;
  ['btn-node','btn-edge'].forEach(id=>document.getElementById(id).className='');
  pill.textContent='select a mode';
  document.getElementById('out-wrap').style.display='none';
  document.getElementById('status').textContent='';
}}
function confirm(){{
  const nl=N.get(),el=E.get();
  if(!nl.length){{alert('Add at least one node.');return;}}
  const idx={{}};nl.forEach((n,i)=>idx[n.id]=i);
  const sz=nl.length;
  const mat=Array.from({{length:sz}},()=>Array(sz).fill(0));
  el.forEach(e=>{{
    const i=idx[e.from],j=idx[e.to];
    const w=WEIGHTED&&e.label?parseFloat(e.label)||1:1;
    mat[i][j]=w;mat[j][i]=w;
  }});
  const pos={{}};
  nl.forEach((nd,i)=>{{pos[i]=[parseFloat((nd.x/100).toFixed(2)),parseFloat((-nd.y/100).toFixed(2))]}});
  const payload=JSON.stringify({{matrix:mat,pos:pos}});
  document.getElementById('out').value=payload;
  document.getElementById('out-wrap').style.display='block';
  document.getElementById('status').textContent=`✓ ${{sz}} nodes · ${{el.length}} edges — copy JSON below`;
}}
function copyOut(){{
...
document.getElementById('copy-btn').addEventListener('click', copyOut);
    ...
    # ── Render the canvas (one-way, display only) ──────────────────────────
    st.components.v1.html(html_code, height=470, scrolling=False)

    # ── JSON input — user pastes/copies confirmed graph here ───────────────
    st.caption("After confirming the graph above, copy the JSON and paste it here:")
    ...
    try:
        ...
        st.session_state[result_key] = {"g": mat, "pos": pos}
...
# ---------------------------------------------------------------------------
# Auto-run demo on first load / experiment switch
# ---------------------------------------------------------------------------
...
# Trigger demo automatically whenever the selected experiment changes
...
with right_col:
    ...
    # ---- Demo result (always shown, auto-populated) ------------------------
    st.markdown("### Demo")
